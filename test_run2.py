...
def upkeep_nb(encounter_phase, encounter_state):
    print(f"REPORT: Round {encounter_state['round']} upkeep")
    encounter_state['round'] += 1
    print(f"MAJOR: Round: {encounter_state['round']}")
    for k in encounter_state['actors']:
        encounter_state['actors'][k]['done'] = False
        encounter_state['actors'][k]['active'] = False
    return 'round', encounter_state

h_encounter.PHASES['upkeep'] = upkeep_nb
